...
const PAPI = {
    ...
    loadKey(appName = null, provider = 'openai') {
        ...
        const hasRealKeys = this.getAnyAvailableKey('openai') || 
    ...
    trackKeyUsage(keyIndex) {
        const currentUsage = parseInt(localStorage.getItem(`key${keyIndex}_usage`) || '0');
     ...
     */
    isPremium() {
        const masterAdmin = localStorage.getItem('master_admin') === 'TROY_WALKER_2026';
    ...
    hasLicense(appName) {
        appName = appName || this.getCurrentApp();
        const licenseName = appName.replace('.html', '').replace(/-/g, '_') + '_license';
     ...
     */
    showKeySetupPrompt() {
        const hasKeys = this.getAnyAvailableKey('openai') || 
                       this.getAnyAvailableKey('claude') || 
                       this.getAnyAvailableKey('gemini');
        
        if (!hasKeys) {
            console.log('ℹ️ Running in demo mode. Configure API keys in Key Controller for full features.');
    ...
    getStats() {
        return {
            ...
            assignment: localStorage.getItem(`assign-${this.getCurrentApp()}`) || 'auto'
        };
    },
    ...
    checkKeys() {
        ...
        console.log(`Claude Key: ${claude ? claude.substring(0, 20) + '...' : '❌ Not set'}`);
        console.log(`Gemini Key: ${gemini ? gemini.substring(0, 15) + '...' : '❌ Not set'}`);
        console.log(`Demo Mode: ${demoMode ? '⚠️ ENABLED' : '✅ Disabled'}`);
        console.log(`Current Key: ${currentKey ? currentKey.substring(0, 20) + '...' : '❌ None'}`);
        console.log(`Master Admin: ${localStorage.getItem('master_admin') === 'TROY_WALKER_2026' ? '👑 ACTIVE' : '❌ Inactive'}`);
...
if (typeof window !== 'undefined') {
    window.addEventListener('DOMContentLoaded', () => {
        if (PAPI.showKeySetupPrompt()) {
            ...
            console.log('✅ PAPI Key Loader ready. Current app:', PAPI.getCurrentApp());
