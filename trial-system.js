...
class TrialManager {
    ...
    initTrial() {
        ...
        if (currentEmail && currentEmail.toLowerCase() === TRIAL_CONFIG.ADMIN_EMAIL) {
            this.adminUser = true;
            return;
        }
        ...
        // Load trial data
        if (this.currentUser) {
            const trialData = JSON.parse(localStorage.getItem('trial_' + this.currentUser) || '{}');
    ...
    trackRequest() {
        ...
        localStorage.setItem('trial_' + this.currentUser, JSON.stringify(this.trialData));

        // Update display
        this.updateRequestCounter();

        // Check if survey should be shown (after 10 total chat sessions)
        const totalChatsUsed = (TRIAL_CONFIG.INITIAL_CHATS + 20) - 
            (this.trialData.chatsRemaining.alien + this.trialData.chatsRemaining.cortex + this.trialData.chatsRemaining.nokn);
        if (totalChatsUsed === TRIAL_CONFIG.SURVEY_THRESHOLD && !this.trialData.surveyCompleted) {
