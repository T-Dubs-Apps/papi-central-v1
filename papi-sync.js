...
class PAPISync {
    ...
    async syncFromCloud() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                console.log(`📅 Backup from: ${new Date(data.timestamp).toLocaleString()}`);
                return true;
            } else {
    ...
    async importBackup(file) {
        try {
            ...
            if (!confirm(`Import ${itemCount} items from backup dated ${new Date(data.timestamp).toLocaleString()}?`)) {
                return false;
            }
    ...
    getSyncStatus() {
        return {
            ...
            itemCount: this.syncKeys.filter(key => localStorage.getItem(key) !== null).length
        };
    }
}
