...
class PAPIFailover {
    ...
    async checkAllServers() {
        ...
        localStorage.setItem('papi_server_status', JSON.stringify(this.serverStatus));
    ...
    recordFailover(fromServer, toServer) {
        ...
        if (this.failoverHistory.length > this.maxFailoverHistory) {
            this.failoverHistory = this.failoverHistory.slice(0, this.maxFailoverHistory);
        }
        
        // Save to localStorage
        localStorage.setItem('papi_failover_history', JSON.stringify(this.failoverHistory));
        
        console.warn(`🔄 FAILOVER: ${failoverEvent.fromName} → ${failoverEvent.toName}`);
    ...
    startMonitoring() {
        ...
        setInterval(async () => {
            ...
            if (this.currentServer && !this.serverStatus[this.currentServer]?.online) {
                console.warn(`⚠️ Current server ${this.getActiveServerName()} is down!`);
                await this.selectBestServer();
            }
        }, this.checkInterval);
    ...
    getServerStatus() {
        return {
            ...
            onlineCount: Object.values(this.serverStatus).filter(s => s.online).length,
    ...
    forceServerSwitch(serverKey) {
        ...
        console.log(`🔄 Manual switch: ${PAPI_SERVERS[oldServer]?.name || 'Unknown'} → ${PAPI_SERVERS[serverKey].name}`);
